...
dataPreview.textContent = JSON.stringify(chartData.slice(0, 4), null, 2);
...
function buildConfig(type, { month, city, metric }) {
  ...
  if (type === "line") return lineOverMonths(city, ["minTempC", "maxTempC"]);
  if (type === "scatter") return scatterHumidityVsFeelLike(city);
...
function lineOverMonths(city, metrics) {
  ...
  return {
    ...
    options: {
      responsive: true,
      plugins: {
        title: { display: true, text: `Trends over months: ${city}` }
      },
      scales: {
